...
T SingleLinkedList<T>::getCurrentVal() const
{
	if (currentPos == nullptr)
		throw "currentPos is nullptr";  //或者用stdlib——perror？
...
{  
   if (currentPos == nullptr)   throw std::runtime_error("currentPos is nullptr");
...
void SingleLinkedList<T>::remove() 
{
	if (currentPos == nullptr )
	{
        return;//throw std::runtime_error("currentPos is nullptr");
	}
    else if (currentPos->next == nullptr) {
        return;//throw std::runtime_error("currentPos->next is nullptr");
    }
